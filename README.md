...
GitHub Actions 없이 내 컴퓨터에서 바로 띄우려면:

```sh
cd StWeb                                                # index.html 이 있는 폴더
...
- **로컬 실행**: 위 `DATA_GO_KR_KEY=발급키` 부분에 인증키를 그대로 붙이세요.
  - 매번 입력하기 귀찮으면 셸 rc 파일(`~/.zshrc` 또는 `~/.bashrc`)에 다음을 추가하고
    새 터미널을 열면 `node scripts/fetch-cheongyak.mjs` 만 쳐도 됩니다.

    ```sh
    export DATA_GO_KR_KEY="여기에-data.go.kr-인증키"
    ```
  - 키는 [data.go.kr 마이페이지 → 인증키 발급현황](https://www.data.go.kr/iim/api/selectAcountList.do) 에서 복사합니다.
    인코딩/디코딩 어느 형태든 OK (스크립트가 알아서 처리).

- **GitHub Pages 배포**: 저장소 → **Settings → Secrets and variables → Actions
  → New repository secret** 에서 이름을 `DATA_GO_KR_KEY` 로 등록합니다.
  (자세한 단계는 위 **"4. 청약 일정 설정"** 섹션 참고.)

> `cheongyak.json` 은 스크립트가 매 실행마다 덮어씁니다. 키 없이 실행하면
> `notices: []` 로 비어있는 채 정상 종료되니, 청약 탭이 비면 키부터 확인하세요.
